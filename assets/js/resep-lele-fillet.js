(function(){
  ...
  async function submitToGoogleForm(values){
    const endpoint = 'https://docs.google.com/forms/d/e/' + encodeURIComponent(GOOGLE_FORM.formId) + '/formResponse';
    const body = new URLSearchParams();
    body.set(GOOGLE_FORM.fields.email, values.email);
    body.set(GOOGLE_FORM.fields.phone, normalizeIndonesianMobile(values.phone));
    body.set(GOOGLE_FORM.fields.rating, values.rating);
    body.set(GOOGLE_FORM.fields.comment, values.comment);
    body.set(GOOGLE_FORM.fields.wish, values.wish);

    // Google Forms tidak menyediakan CORS API publik untuk response submission.
    // mode:no-cors tetap memungkinkan browser mengirimkan form tanpa membuka
    // Google Form atau menampilkan halaman Google kepada customer.
  ...
  if (surveyForm) {
    ...
    surveyForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (surveyStatus) {
          surveyStatus.textContent = 'Survey belum berhasil dikirim. Periksa koneksi internet lalu coba kembali.';
          surveyStatus.style.color = '#b42318';
        }
